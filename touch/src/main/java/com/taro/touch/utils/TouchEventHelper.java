...
public class TouchEventHelper implements View.OnTouchListener {
    @IntDef(value = {EVENT_SINGLE_CLICK_BY_TIME, EVENT_SINGLE_CLICK_BY_DISTANCE})
    ...
    private int SINGLE_CLICK_INTERVAL = 150;
    //双击事件可可持续最长时间间隔(两次单击事件之间的间隔)
    private int DOUBLE_CLICK_INTERVAL = 350;
    //距离单击事件最大的允许偏移量大小
    private int SINGLE_CLICK_OFFSET_DISTANCE = 30;
    ...
    private boolean handleClickEvent(boolean isDistanceEnable, boolean isTimeEnable, int fireEvent, MotionEvent event) {
        ...
        if (this.isFireSingleClickEvent(event, fireEvent)) {
            ...
            if (mIsFireLastClickEvent && mIsDoubleClickEnable) {
                //触摸双击事件
...
//        mIsFireLastClickEvent = false; //此变量不可以重置,这是保存已经完成一次单击事件的标识,用于后续识别双击事件
    }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                //允许触发单点触摸事件
                //这里某设为False可能存在一种情况:
                //如果用户移动了界面,并且使用的时间中够短(在单击检测的时间范围内,即down与up的允许时间范围内)
                //则up事件处理为单击事件,并且为false不处理其它的触摸事件,所以move的最终状态不会被保存
                //也就是up之后move会回到上一次记录的位置(而不是移动后的位置)
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //但实际上这是一次移动事件
                if (mIsClickDistanceEventEnable && !mIsClickDistanceMove) {
    ...
    /**
     * 设置单击有效的时间间隔(down与up事件的最长允许时间间隔),默认150ms
     *
     * @param interval
     */
    ...
    /**
     * 设置单击响应的事件是基于时间的单击事件还是基于距离的单击事件,设置的事件触发时回调,另一个事件则不会回调;<br/>
     * 同一情况下只能有其中一种事件响应(基于时间或者是距离),默认为时间
     *
     * @param enableEventType
     */
    public void setIsEnableSingleClick(@SingleClickEven int enableEventType) {
        boolean isEnabled;
        if (enableEventType == EVENT_SINGLE_CLICK_BY_TIME) {
            isEnabled = true;
        } else {
            isEnabled = false;
        }
     ...
     * @param isEnabled
     */
    public void setIsEnableDoubleClick(boolean isEnabled) {
        this.mIsDoubleClickEnable = isEnabled;
    }


    /**
     * 设置在触发单击事件时是否同时触发单点触摸事件;默认为true
     * <p>单击事件本身属于单点触摸事件之中的一种,只是触摸时间在一定时间以内则认为是单击事件,但同时是满足触发单点触摸事件的(此处仅指up事件)</p>
