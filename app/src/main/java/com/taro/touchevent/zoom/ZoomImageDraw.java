 ...
 */
public class ZoomImageDraw implements TouchEventHelper.OnToucheEventListener, MoveAndScaleTouchHelper.IMoveEvent, MoveAndScaleTouchHelper.IScaleEvent, ICropDrawAction {
    //创建工具类
    private MoveAndScaleTouchHelper mTouch = new MoveAndScaleTouchHelper();
    ...
    private PointF mBmpWH = new PointF();
    private boolean mIsScaleToSmall = false;
    private boolean mIsSet = false;
    private boolean mIsScaleFinished = true;
    ...
    //这个时候就有用了,这个看需要
    public ZoomImageDraw(View drawView) {
    ...
    private void saveOriginalBmpWH(float width, float height) {
        mBmpWH.x = width;
        mBmpWH.y = height;
    }

    public void setScaleBegin() {
        mIsScaleFinished = false;
    }

    public boolean isScaleFinished() {
        return mIsScaleFinished;
    }
    ...
    private boolean isFinishScale(float width, float height, boolean isScaleToSmall) {
        if (isScaleToSmall) {
            if (width >= mBmpWH.x && height >= mBmpWH.y) {
                return true;
            }
        } else {
            if (width <= mBmpWH.x && height <= mBmpWH.y) {
...
//                        + "\ntop=" + mCropRecf.top + "\nbottom=" + mCropRecf.bottom);
                saveOriginalBmpWH(mBitmapRecf.width(), mBitmapRecf.height());
            }
    ...
    public boolean isCanScale(float newScaleRate) {
        ...
        if (!mIsSet) {
            mIsScaleToSmall = newScaleRate < 1;
            mIsSet = true;
        }
        ...
        if (isFinishScale(newWidth, newHeight, mIsScaleToSmall)) {
            mIsScaleFinished = true;
            mIsSet = false;
            return false;
        }
//        if (mTempBmpRectF == null) {
